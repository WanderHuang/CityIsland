...
得到foo_style_css; 也可以通过字符串遍历比较的方式
# 判断数组 #
![没有图片](./pics/17.png)
# DOM的原生基本操作 #
# DOM的重绘、重排 #
 ...
 * 通过cssText优化，element.style.cssText = cssText，用一次复制操作替代多次变动
 * 通过改变节点的class，直接一次性的对节点样式进行切换。
2. 批量修改DOM
 * 先隐藏元素，然后做完更改之后，重新显示元素
 * 将原始元素拷贝到一个脱离文档的节点中，修改副本，最后替换原始元素。
3. 让元素脱离动画流
 * 使用绝对定位
4. 减少使用:hover
 * IE中:hover会严重降低响应速度
...
2. 服务器端渲染能有效提升初始渲染速度。
# Promise #
1. Promise.all用于实现多个异步操作执行完之后进行回调。
实现原理：用一个外层的Promise嵌套，设置一个标志n，内部的promise执行完之后，均执行n++，然后判断n是否等于内部promise的总个数，如果个数相同，则执行外层promise的resolve。
2. Promise的then中如果抛出异常，一样可以在catch中获取到。
3. Generator也是一种异步执行的方式，可以很好的进行步骤的控制，衍生出的async/await实现异步，只是Generator的语法糖实现。
# DOMContentLoaded和onload的区别 #
1. DOMContentLoaded发生在dom构建完成之后，此时页面中的图片、视频等尚未加载完毕。
2. onload会在所有资源加载完毕之后触发。
3. DOMContentLoaded派发的比onload事件更早。
# Service Worker #
　　1、因为js是单线程执行的，Service Worker可以提供一种多线程的编程方式，它是浏览器开启的一个后台线程，可以执行一些离线操作。当js在执行的时候，可以同时使用Service Worker进行其它的操作。
# 快速排序原理 #
　　取一个基准数，比这个数小的，移动到左边，比这个数大的移动到右边。然后重复递归对左右两边进行同样的操作。
# 堆排序的原理 #
